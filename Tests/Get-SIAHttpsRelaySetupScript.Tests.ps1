...
Describe 'Get-SIAHttpsRelaySetupScript' {
    ...
    BeforeEach {
        ...
        $Script:response = Get-SIAHttpsRelaySetupScript -https_relay_os 'linux' -expiration_minutes 30
    }
    ...
    Context 'Request' {
        ...
        It 'sends request with expected body' {
            Should -Invoke -CommandName Invoke-IDRestMethod -ModuleName $Script:SIAModuleName -ParameterFilter {
                ($null -ne $Body) -and ($Body -match 'https_relay_os')
            } -Times 1 -Exactly -Scope It
        }
    }
