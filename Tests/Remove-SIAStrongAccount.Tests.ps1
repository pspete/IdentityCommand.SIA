Describe $($PSCommandPath -Replace '.Tests.ps1') {
    ...
    InModuleScope $(Split-Path (Split-Path (Split-Path -Parent $PSCommandPath) -Parent) -Leaf ) {
        ...
        BeforeEach {
            ...
            $response = $InputObject | Remove-SIAStrongAccount
        ...
        Context 'Input' {
            ...
            It 'sends request to expected endpoint for database strong account' {
                $InputObject | Remove-SIAStrongAccount -database
                Assert-MockCalled Invoke-IDRestMethod -ParameterFilter {

                    $URI -contains 'https://somedomain.dpa.cyberark.cloud/api/adb/secretsmgmt/secrets/SomeID'
